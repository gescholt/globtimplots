...
let proj = joinpath(@__DIR__, "..", "..")
    isfile(joinpath(proj, "Project.toml")) || error(
        "No Project.toml found at $proj. Activate the correct project environment before running this worksheet.",
    )
    Pkg.activate(proj)
end
using GlobtimPostProcessing,
    GlobtimPostProcessing.LV4DAnalysis, GlobtimPlots, GLMakie, DataFrames
GLMakie.activate!()
...
filter = ExperimentFilter(
    gn = fixed(GN),
    degree = sweep(DEG_MIN, DEG_MAX),
    domain = sweep(DOMAIN_MIN, DOMAIN_MAX),
)
df = analyze_sweep(RESULTS_ROOT, filter; export_csv = false, show_header = false)

# === CONVERGENCE RATE ANALYSIS ===
convergence =
    analyze_convergence(RESULTS_ROOT; gn = GN, degree_min = DEG_MIN, degree_max = DEG_MAX)
...
coverage = analyze_coverage(
    RESULTS_ROOT;
    expected_gn = [8],
    expected_domains = [0.08, 0.1, 0.12, 0.15],
    expected_degrees = 8:2:12,
    expected_seeds = 1:5,
)
