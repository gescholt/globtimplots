...
GlobtimPlots is a package in the [globopt_merged](https://github.com/gescholt/globopt_merged)
monorepo, not a standalone registered package. Clone the repo and instantiate the
`viz` (or `dev`) profile, which bundles GlobtimPlots together with Globtim and the
Makie backends:
...
Then launch with `--project=profiles/viz` and `using GlobtimPlots` resolves.
...
[`pkg/globtimplots/examples/gallery.jl`](https://github.com/gescholt/globopt_merged/blob/main/pkg/globtimplots/examples/gallery.jl).
