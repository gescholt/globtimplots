...
function make_capture_result(distances, types, tol_fracs, dd; n_computed = 10)
    ...
    for sym in unique(types)
        ...
        type_rates[sym] =
            [count(captured[ti][idxs]) / length(idxs) for ti in eachindex(tol_fracs)]
    end
    CaptureResult(
        distances,
        nearest,
...
fig2 = plot_capture_sparsification_combined(entries, known; save_path = path2)
println("  Done.")
