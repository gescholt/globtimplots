...
"""
function plot_lv4d_l2_convergence(
    df::DataFrame;
    gn_filter = nothing,
...
"""
function plot_lv4d_recovery_convergence(
    df::DataFrame;
    gn_filter = nothing,
    degree_filter = nothing,
    show_fit::Bool = false,
...
"""
function plot_lv4d_convergence_rate(
    df::DataFrame;
    ...
    title_str =
        "Convergence Rate: $metric_name ∝ domain^α" *
        (gn_filter !== nothing ? " (GN=$gn_filter)" : "")
    ...
    scatter!(ax, degrees[valid_idx], slopes[valid_idx], color = :steelblue, markersize = 12)
    errorbars!(
        ax,
        degrees[valid_idx],
        slopes[valid_idx],
        slope_errors[valid_idx],
        color = :steelblue,
        whiskerwidth = 8,
    )
...
"""
function plot_lv4d_convergence_multi_degree(
    df::DataFrame;
    gn_filter = nothing,
    metric::Symbol = :recovery_error,
    show_fit::Bool = true,
    ...
    title_str =
        "Convergence Analysis: $metric_label" *
        (gn_filter !== nothing ? " (GN=$gn_filter)" : "")
...
L2_norm = ||f - w_d||_L2 = polynomial approximation error
"""
function plot_lv4d_l2_by_degree(
    df::DataFrame;
    gn_filter = nothing,
    ...
    domain_str =
        actual_domain >= 0.01 ? @sprintf("%.4f", actual_domain) :
        @sprintf("%.2e", actual_domain)
    title_str =
        title !== nothing ? title :
...
recovery_error = ||p_best - p_true|| / ||p_true|| (relative parameter error)
"""
function plot_lv4d_recovery_by_degree(
    df::DataFrame;
    gn_filter = nothing,
    domain_filter = nothing,
    show_fit::Bool = true,
...
- `band_type::Symbol=:quantile`: Type of uncertainty band:
  ...
  - `:none` - No band
"""
function _plot_convergence_line!(
    ax,
    df_deg::DataFrame,
    metric_col::Symbol,
    degree::Int,
...
function _fit_convergence_rate(df::DataFrame, metric::Symbol)
    ...
    agg = combine(groupby(df, :domain), actual_metric => mean => :mean_val)
    sort!(agg, :domain)
