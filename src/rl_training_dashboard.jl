...
"""
    plot_training_progress(run; size=(1200, 800))
...
"""
function plot_training_progress(run; size = (1200, 800))
    ...
    Label(
        fig[0, :],
        text = "Training Progress: $strategy_name on $func_name",
        fontsize = 20,
        font = :bold,
    )
    ...
    ax_steps =
        Axis(fig[2, 1], xlabel = "Episode", ylabel = "Steps", title = "Steps per Episode")
...
"""
    plot_action_distribution(run; size=(1000, 600))

Plot action distribution across episodes as stacked bar chart or heatmap
"""
function plot_action_distribution(run; size = (1000, 600))
    episodes = run.episodes
    n_episodes = length(episodes)
    ...
    Label(
        fig[0, :],
        text = "Action Distribution: $strategy_name",
        fontsize = 20,
        font = :bold,
    )
    ...
    hm = heatmap!(ax, 1:n_actions, 1:n_episodes, action_matrix', colormap = :viridis)
...
"""
    plot_l2_error_evolution(run; size=(800, 500))

Plot mean and final L2 errors across episodes
"""
function plot_l2_error_evolution(run; size = (800, 500))
    episodes = run.episodes
    ...
    Label(
        fig[0, :],
        text = "L2 Error Evolution: $strategy_name",
        fontsize = 20,
        font = :bold,
    )
...
"""
    plot_strategy_comparison(runs::Vector; size=(1400, 1000))

Compare multiple training runs (different strategies) on the same function
"""
function plot_strategy_comparison(runs::Vector; size = (1400, 1000))
    if isempty(runs)
    ...
    Label(
        fig[0, :],
        text = "Strategy Comparison on $func_name",
        fontsize = 22,
        font = :bold,
    )
...
"""
    create_training_dashboard(run; size=(1600, 1200))

Create comprehensive dashboard with all training plots
"""
function create_training_dashboard(run; size = (1600, 1200))
    episodes = run.episodes
    n_episodes = length(episodes)
    ...
    Label(
        fig[0, :],
        text = "Training Dashboard: $strategy_name on $func_name",
        fontsize = 24,
        font = :bold,
    )
    ...
    ax_actions.xticklabelrotation = π / 4
...
```
"""
function plot_training_curves_simple(
    results;
    title = "Training Progress",
    expected_minima = nothing,
    window_size = 25,
    fig_size::Tuple{Int,Int} = (1400, 1200),
)
    ...
    lines!(
        ax1,
        episodes,
        rolling_mean(minimizers, window_size),
        color = :steelblue,
        linewidth = 3,
        label = "Rolling Avg ($window_size)",
    )
    ...
    lines!(
        ax2,
        episodes,
        rolling_mean(rewards, window_size),
        color = :orange,
        linewidth = 3,
        label = "Rolling Avg",
    )
    ...
    if !isempty(valid_losses)
        ...
        if length(valid_losses) > window_size
            ...
            lines!(
                ax3,
                valid_indices[1:length(smoothed)],
                smoothed,
                color = :black,
                linewidth = 3,
                linestyle = :dash,
    ...
    text!(
        ...
        1,
        avg_minimizers + 0.2,
        text = @sprintf("%.1f", avg_minimizers),
        align = (:center, :bottom),
        fontsize = 14,
    )
    text!(
    ...
    if haskey(results, :func_config)
        ...
        if haskey(func_config, "f") && haskey(func_config, "domain")
            ...
            if expected_minima !== nothing
                text!(
                    ax5,
                    domain[1] + 0.02 * (domain[2] - domain[1]),
...
```
"""
function plot_multi_strategy_comparison_simple(
    results_dict::Dict;
    expected_minima = 5,
    window_size = 25,
    fig_size::Tuple{Int,Int} = (1600, 1100),
)
    # GLMakie is activated automatically when loaded

    fig = Figure(size = fig_size)

    Label(fig[0, :], text = "Training Strategy Comparison", fontsize = 24, font = :bold)
    ...
    for (i, (name, results)) in enumerate(sort(collect(results_dict)))
        ...
        # Raw data (faint)
        lines!(ax_main, episodes, minimizers, color = (color, 0.2), linewidth = 1)

        # Smoothed curve
        smoothed = rolling_mean(minimizers, window_size)
        lines!(ax_main, episodes, smoothed, label = name, color = color, linewidth = 3)
    end
    ...
    for (i, (name, results)) in enumerate(sort(collect(results_dict)))
        ...
        lines!(ax_reward, episodes, smoothed, label = name, color = color, linewidth = 3)
    end
...
"""
    plot_multifunction_learning_curves(data; action_data=nothing, window=50, size=(1600, 1000), ...)

Plot per-function learning curves for multi-function RL training.

Shows 4-6 panels:
1. Success rate vs episode (per function)
2. Average reward vs episode (per function)
3. Average steps vs episode (per function)
4. Final success rate bar chart
5. Policy evolution: subdivide ratio over time (if action_data provided)
6. Action distribution stacked area chart (if action_data provided)

# Arguments
- `data::Dict{String, NamedTuple}`: Per-function training data with fields:
  - `episode_numbers::Vector{Int}`: Episode indices when function was sampled
  - `success_rates::Vector{Float64}`: Rolling success rate at each episode
  - `rewards::Vector{Float64}`: Reward per episode
  - `steps::Vector{Int}`: Steps per episode
- `action_data::NamedTuple=nothing`: Policy evolution data from get_action_evolution_data()
- `window::Int=50`: Rolling average window size for smoothing
- `size::Tuple{Int,Int}=(1600, 1000)`: Figure size
- `show_raw::Bool=true`: Show raw data points (faint scatter)
- `title::String="Per-Function Learning Curves"`: Figure title
...
# After training with MultiEnv2D:
data = get_learning_curve_data(multi_env; window=50)
fig = plot_multifunction_learning_curves(data)
save("learning_curves.pdf", fig)
```
"""
function plot_multifunction_learning_curves(
    data::Dict{String,<:NamedTuple};
    action_data::Union{NamedTuple,Nothing} = nothing,
    window::Int = 50,
    size::Tuple{Int,Int} = (1600, 1000),
    show_raw::Bool = true,
    title::String = "Per-Function Learning Curves",
)
    if isempty(data)
    ...
    fn_names = sort(collect(keys(data)))
    ...
    # Title
    Label(fig[0, :], text = title, fontsize = 20, font = :bold)

    # Panel 1: Success Rate (top-left)
    ax_sr = Axis(
        fig[1, 1],
        xlabel = "Episode",
        ylabel = "Success Rate",
        ...
        xgridvisible = true,
        ygridvisible = true,
        ytickformat = values -> ["$(Int(round(v*100)))%" for v in values],
    )

    for (i, fn_name) in enumerate(fn_names)
        fn_data = data[fn_name]
        color = colors[mod1(i, length(colors))]
        eps = fn_data.episode_numbers
        sr = fn_data.success_rates
        ...
        # Raw data (faint)
        if show_raw
            scatter!(ax_sr, eps, sr, color = (color, 0.2), markersize = 4)
        end

        # Smoothed line
        smoothed_sr = rolling_mean(sr, min(window, length(sr)))
        lines!(ax_sr, eps, smoothed_sr, color = color, linewidth = 2.5, label = fn_name)

        # Final value annotation
        if !isempty(smoothed_sr)
            final_val = smoothed_sr[end] * 100
            text!(
                ax_sr,
                eps[end],
                smoothed_sr[end],
                text = @sprintf("%.0f%%", final_val),
                fontsize = 9,
                color = color,
                align = (:left, :center),
                offset = (5, 0),
    ...
    for (i, fn_name) in enumerate(fn_names)
        ...
        # Raw data (faint)
        if show_raw
            scatter!(ax_reward, eps, rewards, color = (color, 0.2), markersize = 4)
        end

        # Smoothed line
        smoothed_r = rolling_mean(rewards, min(window, length(rewards)))
        lines!(ax_reward, eps, smoothed_r, color = color, linewidth = 2.5, label = fn_name)
    end

    # Panel 3: Steps (bottom-left)
    ax_steps = Axis(
        fig[2, 1],
        xlabel = "Episode",
        ylabel = "Steps",
        title = "Steps Per Episode (lower = more efficient)",
        xgridvisible = true,
        ygridvisible = true,
    )
    ...
    for (i, fn_name) in enumerate(fn_names)
        ...
        # Raw data (faint)
        if show_raw
            scatter!(ax_steps, eps, steps, color = (color, 0.2), markersize = 4)
        end

        # Smoothed line
        smoothed_s = rolling_mean(steps, min(window, length(steps)))
        lines!(ax_steps, eps, smoothed_s, color = color, linewidth = 2.5, label = fn_name)
    end

    # Panel 4: Final Success Rate Bar Chart (bottom-right)
    # Compute final success rates for each function
    final_success_rates = Float64[]
    bar_colors = []
    for (i, fn_name) in enumerate(fn_names)
        fn_data = data[fn_name]
        final_sr = isempty(fn_data.success_rates) ? 0.0 : fn_data.success_rates[end] * 100
        push!(final_success_rates, final_sr)
        push!(bar_colors, colors[mod1(i, length(colors))])
    end
    ...
    ax_bars = Axis(
        ...
        xlabel = "Success Rate",
        yticks = (1:length(fn_names), fn_names),
        limits = (0, 105, 0.5, length(fn_names) + 0.5),
        xgridvisible = true,
        xtickformat = values -> ["$(Int(round(v)))%" for v in values],
    )
    ...
    for (i, sr) in enumerate(final_success_rates)
        text!(
            ax_bars,
            sr + 2,
            i,
    ...
    # Policy Evolution Panels (if action data provided)
    legend_row = 3
    if action_data !== nothing && !isempty(action_data.episode_numbers)
        legend_row = 4  # Move legend down

        # Panel 5: Action Ratio Evolution (row 3, col 1)
        ax_ratio = Axis(
            fig[3, 1],
            xlabel = "Episode",
            ylabel = "Subdivide Ratio",
            title = "Policy Evolution: Subdivide vs Degree",
        ...
        lines!(
            ax_ratio,
            action_data.episode_numbers,
            action_data.subdivide_ratio,
            color = :blue,
            linewidth = 2.5,
        )
        hlines!(ax_ratio, [0.5], color = :gray, linestyle = :dash, linewidth = 1)
        text!(
            ax_ratio,
            action_data.episode_numbers[end],
            action_data.subdivide_ratio[end],
            text = @sprintf("%.0f%%", action_data.subdivide_ratio[end] * 100),
            fontsize = 9,
            color = :blue,
            align = (:left, :center),
            offset = (5, 0),
        ...
        # Panel 6: Stacked Area Chart (row 3, col 2)
        ax_stack = Axis(
            fig[3, 2],
            xlabel = "Episode",
            ylabel = "Proportion",
        ...
        eps = action_data.episode_numbers
        props = action_data.action_proportions

        # Stacked bands (bottom to top: accept, degree, override, subdivide)
        y0 = zeros(length(eps))
        y1 = props.accept
        y2 = y1 .+ props.degree
        y3 = y2 .+ props.override
        y4 = y3 .+ props.subdivide

        band!(ax_stack, eps, y0, y1, color = (:green, 0.7), label = "Accept")
        band!(ax_stack, eps, y1, y2, color = (:orange, 0.7), label = "Degree+")
        band!(ax_stack, eps, y2, y3, color = (:purple, 0.7), label = "Override")
        band!(ax_stack, eps, y3, y4, color = (:blue, 0.7), label = "Subdivide")

        axislegend(ax_stack, position = :rt, framevisible = true, labelsize = 10)
    end

    # Shared legend at bottom (horizontal layout)
    Legend(
        fig[legend_row, :],
        ax_sr,
