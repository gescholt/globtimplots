...
function plot_multifunction_learning_curves(
    ...
    for (i, fn_name) in enumerate(fn_names)
        ...
        if !isempty(smoothed_sr)
            ...
            text!(ax_sr, eps[end], smoothed_sr[end],
                  text=@sprintf("%.0f%%", final_val),
                  fontsize=9, color=color, align=(:left, :center), offset=(5, 0))
        end
    end

    # Panel 2: Reward (top-right)
    ax_reward = Axis(fig[1, 2],
                     xlabel="Episode",
                     ylabel="Reward",
    ...
    barplot!(ax_bars, 1:length(fn_names), final_success_rates,
             direction=:x, color=bar_colors, strokewidth=1, strokecolor=:black)

    # Add percentage labels at end of each bar
    for (i, sr) in enumerate(final_success_rates)
        text!(ax_bars, sr + 2, i,
              text=@sprintf("%.0f%%", sr),
              fontsize=11, align=(:left, :center))
    end
    ...
    if action_data !== nothing && !isempty(action_data.episode_numbers)
        ...
        lines!(ax_ratio, action_data.episode_numbers, action_data.subdivide_ratio,
               color=:blue, linewidth=2.5)
        hlines!(ax_ratio, [0.5], color=:gray, linestyle=:dash, linewidth=1)
        text!(ax_ratio, action_data.episode_numbers[end], action_data.subdivide_ratio[end],
              text=@sprintf("%.0f%%", action_data.subdivide_ratio[end] * 100),
              fontsize=9, color=:blue, align=(:left, :center), offset=(5, 0))

        # Panel 6: Stacked Area Chart (row 3, col 2)
        ax_stack = Axis(fig[3, 2],
                        xlabel="Episode",
                        ylabel="Proportion",
    ...
    Legend(fig[legend_row, :], ax_sr, orientation=:horizontal, tellwidth=false, tellheight=true,
           framevisible=true, labelsize=12, nbanks=1)
