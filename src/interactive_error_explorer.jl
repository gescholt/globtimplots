...
"""
function _compute_error_grid_in_window(
...
function _regenerate_error!(obs_state; resolution::Int = obs_state.resolution)
    tree = obs_state.tree
    f = obs_state.f
    ...
    @printf(
        "  Reset: %d×%d grid  domain=[%.3g,%.3g]×[%.3g,%.3g]\n",
        resolution,
        resolution,
        x_lo,
        x_hi,
...
"""
function _refine_error!(obs_state, ax; resolution::Int = obs_state.resolution)
    tree = obs_state.tree
    f = obs_state.f
    ...
    @printf(
        "  Refined: %d×%d grid  x=[%.3g,%.3g]  y=[%.3g,%.3g]  zoom≈%.1f×%.1f\n",
        resolution,
        resolution,
        x_min,
        x_max,
...
function interactive_error_explorer(
    ...
    length(root_bounds) == 2 ||
        error("interactive_error_explorer requires a 2D subdivision tree")
