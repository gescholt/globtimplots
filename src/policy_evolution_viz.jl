...
function plot_action_ratio_evolution(run; fig_size = (1000, 600))
    ...
    if window > 0 && n_episodes > window
        smoothed = [
            mean(ratios[max(1, i - window):min(n_episodes, i + window)]) for
            i in 1:n_episodes
        ]
    else
        smoothed = ratios
    ...
    Label(
        fig[0, :],
        text = "Action Ratio Evolution: $strategy_name on $func_name",
        fontsize = 20,
        font = :bold,
    )
...
function plot_action_stacked_area(run; fig_size = (1000, 600))
    ...
    Label(
        fig[0, :],
        text = "Action Distribution Evolution: $strategy_name on $func_name",
        fontsize = 20,
        font = :bold,
    )
    ...
    for (i, action) in enumerate(action_names)
        ...
        band!(ax, ep_nums, cumulative, upper, color = (color, 0.6), label = action)
...
function plot_state_action_heatmap(state_action_pairs; fig_size = (1200, 800))
    ...
    Label(
        fig[0, :],
        text = "Policy State-Action Map: SUBDIVIDE vs INCREASE_DEGREE",
        fontsize = 20,
        font = :bold,
    )
...
function plot_policy_evolution_comparison(runs::Vector; fig_size = (1400, 1000))
    ...
    Label(
        fig[0, :],
        text = "Policy Evolution Comparison on $func_name",
        fontsize = 22,
        font = :bold,
    )
    ...
    if all(run -> !isnothing(run.aggregate), runs)
        ...
        mean_subdivide =
            [mean([get(ep.actions, "SUBDIVIDE", 0) for ep in run.episodes]) for run in runs]
        mean_degree = [
            mean([get(ep.actions, "INCREASE_DEGREE", 0) for ep in run.episodes]) for
            run in runs
        ]
...
"""
function plot_episode_decision_timeline(
    episode_metrics,
    ...
    ax_degree =
        Axis(fig[2, 1], ylabel = "Polynomial Degree", title = "Approximation Complexity")
    ...
    action_map =
        Dict("SUBDIVIDE" => 1, "INCREASE_DEGREE" => 2, "COMPUTE_MINIMA" => 3, "DONE" => 4)
    action_ints = [get(action_map, a, 0) for a in actions]

    # Color code by action type
    colors_actions = [
        a == "SUBDIVIDE" ? :red :
        a == "INCREASE_DEGREE" ? :blue : a == "COMPUTE_MINIMA" ? :green : :gray for
        a in actions
    ]

    scatter!(ax_actions, steps, action_ints, color = colors_actions, markersize = 12)
